...
class CrossAttention(nn.Module):
    ...
    def __init__(
        ...
        self.to_self_k = nn.Linear(query_dim, inner_dim, bias=False)
        self.to_self_v = nn.Linear(query_dim, inner_dim, bias=False)
    ...
    def forward(self, x: torch.Tensor, context: torch.Tensor | None = None) -> torch.Tensor:
        ...
        if context.max() == 0:
            context = x.clone()
            key = self.to_self_k(context)
            value = self.to_self_v(context)
        else:
