...
class BrownianBridgeModel(nn.Module):
    ...
    def p_losses(self, x0, y, context, t, noise=None):
        ...
        objective_recon = self.denoise_fn(torch.cat([y, x_t], dim=1), timesteps=t, context=context)
    ...
    def p_sample(self, x_t, y, context, i, clip_denoised=False):
        ...
        if self.steps[i] == 0:
            ...
            x0_recon = self.predict_x0_from_objective(x_t[:,y.shape[1]:], y, t, objective_recon=objective_recon)
            if clip_denoised:
                x0_recon.clamp_(-1., 1.)
    ...
    def p_sample_loop(self, y, context=None, clip_denoised=True, sample_mid_step=False):
        ...
        if sample_mid_step:
            imgs, one_step_imgs = [torch.cat([y,y], dim=1)], []
            for i in tqdm(range(len(self.steps)), desc=f'sampling loop time step', total=len(self.steps)):
                img, x0_recon = self.p_sample(x_t=imgs[-1], y=y, context=context, i=i, clip_denoised=clip_denoised)
                img = torch.cat([y, img], dim=1)
                imgs.append(img)
                one_step_imgs.append(x0_recon)
            imgs = [img[:,y.shape[1]:] for img in imgs]
            return imgs, one_step_imgs
        else:
            img = torch.cat([y,y], dim=1)
            for i in tqdm(range(len(self.steps)), desc=f'sampling loop time step', total=len(self.steps)):
