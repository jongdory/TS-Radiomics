...
from model.VQGAN_monai.vector_quantizer import EMAQuantizer, VectorQuantizer
...
class Interpolation(nn.Module):
    def __init__(self, scale_factor=None, mode='trilinear', align_corners=False, padding_size=None):
    ...
    def forward(self, x):
        size = tuple(a+b for a,b in zip(x.shape[2:], self.padding_size))
...
class Decoder(nn.Module):
    ...
    def __init__(
        ...
        for i in range(len(self.num_channels)):
            ...
            # if i > 0:
            #     blocks.append(Interpolation(mode="nearest", padding_size=(0,1,0)))
...
class VQVAE(nn.Module):
    ...
    def init_from_ckpt(self, path, ignore_keys=list()):
        ...
        self.load_state_dict(new_autoencoder_state_dict, strict=False)
