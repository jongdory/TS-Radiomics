...
# from datasets.brats import get_brats_dataset, Sampler
...
class BaseRunner(ABC):
    ...
    def on_save_checkpoint(self, net, train_loader, epoch, step):
        """
                    ...
                    if len(self.optimizer) > 1:
                        pbar.set_description(
                            (
                                ...
                                f'iter: {self.global_step} loss-1: {losses[0].item():.4f} loss-2: {losses[1].item():.4f}'
                            )
                        )
                    else:
                        pbar.set_description(
                            (
                                f'Epoch: [{epoch + 1} / {self.config.training.n_epochs}] '
                                f'iter: {self.global_step} loss: {losses[0].item():.6f} lr:{self.optimizer[0].param_groups[0]["lr"]:.6f}'
                            )
                        )
                ...
                if (epoch + 1) % self.config.training.validation_interval == 0 or (
                    ...
                    if not self.config.training.use_DDP or \
                        ...
                        with torch.no_grad():
                            ...
                            average_loss = self.validation_epoch(train_loader, epoch)
                            torch.cuda.empty_cache()
                ...
                if (epoch + 1) % self.config.training.save_interval == 0 or \
                    ...
                    if not self.config.training.use_DDP or \
                        ...
                        with torch.no_grad():
                            ...
                            self.on_save_checkpoint(self.net, train_loader, epoch, self.global_step)
