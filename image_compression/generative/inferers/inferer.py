...
class DiffusionInferer(Inferer):
    ...
    def sample(
        ...
        for t in progress_bar:
            ...
            image, _ = scheduler.step(model_output, t, image[:,4:])

            # model_output = torch.cat([moving, model_output], dim=1)
...
class VQVAETransformerInferer(Inferer):
    ...
    def __call__(
        ...
        # if transformer_model is a distributed model
        if hasattr(transformer_model, "module"):
            max_seq_len = transformer_model.module.max_seq_len
        else:
    ...
    def sample(
        ...
        for _ in progress_bar:
            ...
            if latent_seq.size(1) <= max_seq_len:
                idx_cond = latent_seq
            else:
                idx_cond = latent_seq[:, -max_seq_len :]
