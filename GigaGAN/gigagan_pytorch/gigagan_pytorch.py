...
from gigagan_pytorch.radiomics_torch import RadiomicsShape
...
def conv3d_3x3(dim_in, dim_out):
    return nn.Conv3d(dim_in, dim_out, 3, padding = 1)
...
class ChannelRMSNorm(nn.Module):
    ...
    def forward(self, x):
        normed = F.normalize(x, dim = 1, eps=1e-7)
...
class PixelShuffle3D(nn.Module):
    ...
    def forward(self, x):
        ...
        input_view = x.contiguous().view(
            batch_size, channels, self.upscale_factor, self.upscale_factor, self.upscale_factor, in_depth, in_height, in_width)
...
class PixelShuffleUpsample(nn.Module):
    ...
    def init_conv_(self, conv):
        ...
        conv_weight = torch.empty(o // 8, i, h, w, d)
...
def get_same_padding(size, kernel, dilation, stride):
    padding_depth = ((size[0] - 1) * (stride - 1) + dilation * (kernel - 1)) // 2
    padding_height = ((size[1] - 1) * (stride - 1) + dilation * (kernel - 1)) // 2
    padding_width = ((size[2] - 1) * (stride - 1) + dilation * (kernel - 1)) // 2

    return (padding_depth, padding_height, padding_width)

class AdaptiveConv3DMod(nn.Module):
    def __init__(
        self,
        dim,
        dim_out,
        ...
        *,
        ...
        self.weights = nn.Parameter(torch.randn((num_conv_kernels, dim_out, dim, kernel, kernel, kernel)))
    ...
    def forward(
        ...
        b, d, h, w = fmap.shape[0], *fmap.shape[-3:]
        ...
        if self.adaptive:
            ...
            assert kernel_mod is not None

            kernel_attn = kernel_mod.softmax(dim=-1)
            kernel_attn = rearrange(kernel_attn, 'b n -> b n 1 1 1 1 1')
        ...
        if self.demod:
            inv_norm = reduce(weights ** 2, 'b o i k1 k2 k3 -> b o 1 1 1 1', 'sum').clamp(min=self.eps).rsqrt()
            weights = weights * inv_norm

        fmap = rearrange(fmap, 'b c d h w -> 1 (b c) d h w')
        weights = rearrange(weights, 'b o ... -> (b o) ...')
        padding = get_same_padding((d, h, w), self.kernel, self.dilation, self.stride)
        fmap = F.conv3d(fmap, weights, padding=padding, groups=b)
...
class SelfAttention(nn.Module):
    def __init__(
        ...
        self.norm = ChannelRMSNorm(dim) # Assuming ChannelRMSNorm is also modified for 3D

        self.to_q = nn.Conv3d(dim, dim_inner, 1, bias=False)
        self.to_k = nn.Conv3d(dim, dim_inner, 1, bias=False) if dot_product else None
        self.to_v = nn.Conv3d(dim, dim_inner, 1, bias=False)

        self.null_kv = nn.Parameter(torch.randn(2, heads, dim_head))

        self.to_out = nn.Conv3d(dim_inner, dim, 1, bias=False)
    ...
    def forward(self, fmap):
        ...
        k = self.to_k(fmap) if self.to_k is not None else q
        q, k, v = map(lambda t: rearrange(t, 'b (h d) x y z -> (b h) (x y z) d', h=self.heads), (q, k, v))
        ...
        out = rearrange(out, '(b h) (x y z) d -> b (h d) x y z', x=x, y=y, z=z, h=h)
...
class CrossAttention(nn.Module):
    def __init__(
        ...
        self.scale = dim_head ** -0.5
        dim_inner = dim_head * heads
        kv_input_dim = default(dim_context, dim) # Note: `default` is not defined in the provided code. Make sure to import or define it.

        self.norm = ChannelRMSNorm(dim)  # Assuming ChannelRMSNorm is also modified for 3D
        # self.norm_context = RMSNorm(kv_input_dim)  # Assuming RMSNorm handles 3D
    ...
    def forward(self, fmap, context, mask=None):
        ...
        q = rearrange(q, 'b (h d) x y z -> (b h) (x y z) d', h=self.heads)
...
def FeedForward(
    ...
    proj = partial(nn.Conv3d, kernel_size = 1) if channel_first else nn.Linear
...
class Generator(BaseGenerator):
    ...
    def __init__(
        ...
        *,
        ...
        adaptive_conv = partial(AdaptiveConv3DMod, kernel = 3, num_conv_kernels = num_conv_kernels)

        self.ls = latent_size
        self.dim_latent = dim_latent
        self.init_block = nn.Parameter(torch.randn(dim_latent, self.ls[0], self.ls[1], self.ls[2]))
        self.init_conv = conv3d_3x3(dim_latent, dim_latent) #adaptive_conv
        ...
        self.layers = nn.ModuleList([])
        norm_klass = ChannelRMSNorm # nn.GroupNorm(32, dim_out)
        ...
        for ind, ((dim_in, dim_out), resolution) in enumerate(zip(dim_pairs, resolutions)):
            ...
            resnet_block = nn.Sequential(
                conv3d_3x3(dim_in, dim_out), # adaptive_conv
                norm_klass(dim_out) ,# nn.GroupNorm(dim_capacity*2, dim_out),
                leaky_relu(),
                conv3d_3x3(dim_out, dim_out), # adaptive_conv
                norm_klass(dim_out), # nn.GroupNorm(dim_capacity*2, dim_out), 
                leaky_relu()
            )

            to_rgb = conv3d_3x3(dim_out, channels) # adaptive_conv
    ...
    def forward(
        ...
        init_block = torch.randn(self.dim_latent, self.ls[0], self.ls[1], self.ls[2]).to(self.device)
        x = repeat(init_block, 'c h w d -> b c h w d', b = batch_size)
        # x = self.init_conv(x, mod = next(conv_mods), kernel_mod = next(conv_mods))
        x = self.init_conv(x)
        rgb = torch.zeros((batch_size, self.channels, self.ls[0], self.ls[1], self.ls[2]), device = self.device, dtype = x.dtype)
        # rgb = torch.zeros((batch_size, self.channels, 4, 4, 4), device = self.device, dtype = x.dtype)
        excitations = [None] * self.num_skip_layers_excite

        rgbs = []
        for squeeze_excite, resnet_block, to_rgb_conv, self_attn, cross_attn, upsample, upsample_rgb in self.layers:
            if exists(upsample):
                x = upsample(x)
            ...
            layer_rgb = to_rgb_conv(x)
            rgb = rgb + layer_rgb

            rgbs.append(rgb)
            if exists(upsample_rgb):
                rgb = upsample_rgb(F.sigmoid(rgb))
...
class SimpleDecoder(nn.Module):
    def __init__(
        ...
        *,
        ...
        # norm_klass = ChannelRMSNorm

        dims = [dim, *dims]

        # dim_capacity = dims[-2]*2

        layers = [conv3d_3x3(dim, dim)]

        for dim_in, dim_out in zip(dims[:-1], dims[1:]):
            layers.append(nn.Sequential(
                Upsample(dim_in),
                conv3d_3x3(dim_in, dim_out),
                leaky_relu()))
                # norm_klass(dim_out)),
                
            # if dim_out % dim_capacity == 0:
            #     layers.append(
            #         nn.GroupNorm(dim_capacity, dim_out), # norm_klass(dim_out),
            #     )
    ...
    def forward(
        ...
        if self.frac_patches < 1.:
            ...
            fmap, orig_image = map(lambda t: rearrange(t, 'b c (p1 h) (p2 w) (p3 d) -> b (p1 p2 p3) c h w d', p1 = patch_dim, p2 = patch_dim, p3 = patch_dim), (fmap, orig_image))
...
class VisionAidedDiscriminator(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        conv_klass = partial(AdaptiveConv3DMod, kernel = 3, num_conv_kernels = num_conv_kernels) if not unconditional else conv3d_3x3
    ...
    def forward(
        ...
        for layer_index, (rand_proj, conv, to_conv_mod, to_conv_kernel_mod, to_logits) in zip(self.layer_indices, self.layer_discriminators):
            ...
            img_fmap = rearrange(rest_tokens, 'b (h w z) d -> b d h w z', h = height_width)
...
class Predictor(nn.Module):
    def __init__(
        ...
        super().__init__()
        self.unconditional = unconditional
        self.residual_fn = nn.Conv3d(dim, dim, 1)
        self.residual_scale = 2 ** -0.5

        self.layers = nn.ModuleList([])

        klass = nn.Conv3d if unconditional else partial(AdaptiveConv3DMod, num_conv_kernels = num_conv_kernels)
        ...
        self.to_logits = nn.Conv3d(dim, 1, 1)
    ...
    def forward(
        ...
        for conv1, activation in self.layers:
...
class Discriminator(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        for ind, ((dim_in, dim_out), resolution) in enumerate(zip(dim_pairs, resolutions)):
            ...
            from_rgb = nn.Conv3d(channels, dim_in, 7, padding = 3)

            # residual convolution

            residual_conv = nn.Conv3d(dim_in, dim_out, 1, stride = (2 if should_downsample else 1))
        ...
        gap3d = partial(nn.AdaptiveAvgPool3d, output_size=(1,1,1))
        flatten = nn.Flatten(1, -1)
        latent_dim = dim_last * (2 ** 3) * 4 if latent_size == (8,8,6) else dim_last * (2 ** 3)
        if latent_size == (2,2,2) or latent_size == (3,3,2):
            layers = [
                nn.Conv3d(dim_last, dim_last, 1, padding = 0),
                gap3d(),
                leaky_relu(),
        ...
        elif latent_size == (3,3,3) or latent_size == (4,4,4) or latent_size == (4,4,3):
            layers = [
                nn.Conv3d(dim_last, dim_last, 3, padding = 0),
                gap3d(),
                leaky_relu(),
                Rearrange('b c h w d -> b (c h w d)'),
                nn.Linear(dim_last, 1),
        ...
        self.to_logits = nn.Sequential(*layers)

        if not unconditional:
            if isinstance(text_encoder, dict):
                text_encoder = TextEncoder(**text_encoder)

            self.text_dim = default(text_dim, 1)
    ...
    def forward(
        self,
        images,
        rgbs= None,                   # multi-resolution inputs (rgbs) from the generator
        texts = None,
        text_encodings: Optional[Tensor] = None,
        text_embeds = None,
        ...
        if exists(texts):
            ...
            conv_mods = self.text_to_conv_conditioning(texts).split(self.predictor_dims, dim = -1)
...
class GigaGAN(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        calc_radiomics_loss_every = 20,
        resize_image_mode = 'trilinear',
        ...
        radiomics_path = '/store8/05.IntracranialArtery/radiomics_features/brats_shape_features-tumor.csv',
    ):
        super().__init__()
        
        df = pd.read_csv(radiomics_path, index_col=0)
        self.means = df.mean()
        self.stds = df.std()
        ...
        self.calc_radiomics_loss_every = calc_radiomics_loss_every
    ...
    def load(self, path, strict = False):
        ...
        g_load_state_dict = OrderedDict()
        for k, v in pkg['G'].items():
            if k in self.unwrapped_G.state_dict() and v.shape == self.unwrapped_G.state_dict()[k].shape:
                g_load_state_dict[k] = v
        self.unwrapped_G.load_state_dict(g_load_state_dict, strict = strict)
        d_load_state_dict = OrderedDict()
        for k, v in pkg['D'].items():
            if k in self.unwrapped_D.state_dict() and v.shape == self.unwrapped_D.state_dict()[k].shape:
                d_load_state_dict[k] = v
        self.unwrapped_D.load_state_dict(d_load_state_dict, strict = strict)
        ...
        if self.has_ema_generator:
            g_ema_load_state_dict = OrderedDict()
            for k, v in pkg['G_ema'].items():
                if self.G_ema.state_dict().get(k) is not None and v.shape == self.G_ema.state_dict()[k].shape:
                    g_ema_load_state_dict[k] = v
            self.G_ema.load_state_dict(g_ema_load_state_dict, strict = strict)
    ...
    def generate_kwargs(self, dl_iter, batch_size):
        ...
        if self.train_upsampler:
            ...
            lowres_real_images = F.interpolate(real_images, (size, size, size))
            # sitk.WriteImage(sitk.GetImageFromArray(real_images[0,0].cpu().numpy()), 'real_image0.nii.gz')
            # sitk.WriteImage(sitk.GetImageFromArray(lowres_real_images[0,0].cpu().numpy()), 'lowres_real_image0.nii.gz')
            # sitk.WriteImage(sitk.GetImageFromArray(real_images[0,1].cpu().numpy()), 'real_image1.nii.gz')
            # sitk.WriteImage(sitk.GetImageFromArray(lowres_real_images[0,1].cpu().numpy()), 'lowres_real_image1.nii.gz')
        ...
        return G_kwargs, maybe_text_kwargs, real_images
    ...
    def train_discriminator_step(
        ...
        for _ in range(grad_accum_every):
            ...
            # real_images, radiomics = real_images

            real_images = real_images.to(self.device)
            real_images.requires_grad_()

            # if self is distributed
            if hasattr(self.D, 'module'):
                real_images_rgbs = [self.resize_image_to(real_images, resolution) for resolution in self.D.module.multiscale_input_resolutions]
            else:
            ...
            G_kwargs, maybe_text_kwargs, _ = self.generate_kwargs(dl_iter, batch_size)
            ...
            with self.accelerator.autocast():
                batch_size, channels, _, _, _ = images.shape
    ...
    def train_generator_step(
        ...
        grad_accum_every = 1,
        calc_multiscale_loss = True,
        calc_radiomics_loss = False
    ):
        total_divergence = 0.
        total_multiscale_divergence = 0. if calc_multiscale_loss else None
        total_radiomics_loss = 0. #if calc_radiomics_loss else None
        ...
        for _ in range(grad_accum_every):
            ...
            with self.accelerator.autocast():
                ...
                # radiomics loss for generator
                if calc_radiomics_loss:
                    radiomics_pred = self.shapeFeaturesExtractor(images)
                    radiomics_pred = torch.tensor(radiomics_pred[None,:], dtype=torch.float32, device=self.device).to(self.device)
                    radiomics_true = torch.tensor(maybe_text_kwargs['texts'], dtype=torch.float32, device=self.device).to(self.device)

                    rad_loss = F.mse_loss(radiomics_pred, radiomics_true)
                    total_radiomics_loss = (rad_loss.item() / grad_accum_every)
                    total_loss = total_loss + rad_loss * 4
    ...
    def shapeFeaturesExtractor(self, img):
        img = img.clone()
        img_r = torch.zeros_like(img[0,0]).to(self.device)
        if 'brats' in str(self.results_folder).split('/')[-1]:
            img_r[img[0,1] > 0.5] = 4
            img_r[img[0,0] > 0.5] = 1
        else:
            img_r[img[0,0] > 0.5] = 1
        ...
        roi = (img_r == 1)
        # Shape 17 features
        shapeFeatures = RadiomicsShape(img_r, roi, binCount=256, verbose=True, interpolator=None)
        shapeFeatures._initSegmentBasedCalculation()

        shapeFeaturesDict.append(shapeFeatures.getMeshVolumeFeatureValue())
        shapeFeaturesDict.append(shapeFeatures.getVoxelVolumeFeatureValue())
        shapeFeaturesDict.append(shapeFeatures.getSurfaceAreaFeatureValue())
        shapeFeaturesDict.append(shapeFeatures.getSurfaceVolumeRatioFeatureValue())
        shapeFeaturesDict.append(shapeFeatures.getSphericityFeatureValue())
        shapeFeaturesDict.append(shapeFeatures.getCompactness1FeatureValue())
        shapeFeaturesDict.append(shapeFeatures.getCompactness2FeatureValue())
        shapeFeaturesDict.append(shapeFeatures.getSphericalDisproportionFeatureValue())
        shapeFeaturesDict.append(shapeFeatures.getMaximum3DDiameterFeatureValue())
        shapeFeaturesDict.append(shapeFeatures.getMaximum2DDiameterSliceFeatureValue())
        shapeFeaturesDict.append(shapeFeatures.getMaximum2DDiameterColumnFeatureValue())
        shapeFeaturesDict.append(shapeFeatures.getMaximum2DDiameterRowFeatureValue())
        shapeFeaturesDict.append(shapeFeatures.getMajorAxisLengthFeatureValue())
        shapeFeaturesDict.append(shapeFeatures.getMinorAxisLengthFeatureValue())
        shapeFeaturesDict.append(shapeFeatures.getLeastAxisLengthFeatureValue())
        shapeFeaturesDict.append(shapeFeatures.getElongationFeatureValue())
        shapeFeaturesDict.append(shapeFeatures.getFlatnessFeatureValue())

        if 'brats' in str(self.results_folder).split('/')[-1]:
            roi = (img_r == 1) + (img_r == 4)

            shapeFeatures = radiomics.shape.RadiomicsShape(img_r, roi, binCount=256, verbose=True, interpolator=None)
            shapeFeatures._initSegmentBasedCalculation()
        ...
        z_scores = (shapeFeaturesDict - self.means) / self.stds

        return z_scores.values

    def sample(self, dl_iter, batch_size):
        G_kwargs, maybe_text_kwargs, real_images = self.generate_kwargs(dl_iter, batch_size)

        with self.accelerator.autocast():
            generator_output = self.G(**G_kwargs, **maybe_text_kwargs)

        if not self.train_upsampler:
            return torch.cat([real_images, generator_output])
        d, h, w  = generator_output.shape[2:]
        lowres_image = G_kwargs['lowres_image']
        lowres_image = F.interpolate(lowres_image, (d, h, w))

        return torch.cat([real_images, lowres_image, generator_output])
    ...
    def save_sample(
        ...
        sample_models_and_output_file_name = [(self.G, f'sample-{milestone}')]

        if self.has_ema_generator:
            sample_models_and_output_file_name.append((self.G_ema, f'ema-sample-{milestone}'))
        ...
        for model, filename in sample_models_and_output_file_name:
            ...
            all_images_numpy = all_images.detach().cpu().numpy()
            for i, image_numpy in enumerate(all_images_numpy):
                save_numpy = np.zeros_like(image_numpy[0])
                # save_numpy = image_numpy[0]
                if 'brats' in str(self.results_folder).split('/')[-1]:
                    # save_numpy[image_numpy[2] > 0.5] = 2
                    save_numpy[image_numpy[1] > 0.5] = 4
                    save_numpy[image_numpy[0] > 0.5] = 1
                    sitk.WriteImage(sitk.GetImageFromArray(image_numpy[0]), os.path.join(self.results_folder, f'{filename}_prob0_{i}.nii.gz'))
                    sitk.WriteImage(sitk.GetImageFromArray(image_numpy[1]), os.path.join(self.results_folder, f'{filename}_prob1_{i}.nii.gz'))
                elif 'lung' in str(self.results_folder).split('/')[-1] or 'kidney' in str(self.results_folder).split('/')[-1]:
                    save_numpy[image_numpy[0] > 0.5] = 1
                    sitk.WriteImage(sitk.GetImageFromArray(image_numpy[0]), os.path.join(self.results_folder, f'{filename}_prob_{i}.nii.gz'))
                elif 'breast' in str(self.results_folder).split('/')[-1]:
                    save_numpy[image_numpy[1] > 0.5] = 1
                    save_numpy[image_numpy[0] > 0.5] = 2
                    sitk.WriteImage(sitk.GetImageFromArray(image_numpy[0]), os.path.join(self.results_folder, f'{filename}_prob_{i}.nii.gz'))
                sitk.WriteImage(sitk.GetImageFromArray(save_numpy), os.path.join(self.results_folder, f'{filename}_{i}.nii.gz'))
    ...
    def forward(
        ...
        *,
        ...
        for _ in tqdm(range(steps), initial = self.steps.item()):
            ...
            calc_radiomics_loss =  self.calc_radiomics_loss_every > 0 and divisible_by(steps, self.calc_radiomics_loss_every)
            (
                d_loss,
                multiscale_d_loss,
                vision_aided_d_loss,
            ...
            (
                ...
                grad_accum_every = grad_accum_every,
                calc_multiscale_loss = calc_multiscale_loss,
                calc_radiomics_loss = calc_radiomics_loss
            )
