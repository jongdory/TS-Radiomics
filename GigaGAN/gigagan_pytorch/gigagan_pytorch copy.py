...
class AdaptiveConv3DMod(nn.Module):
    ...
    def forward(
        ...
        # padding = (get_same_padding(d, self.kernel, self.dilation, self.stride),
        #            get_same_padding(h, self.kernel, self.dilation, self.stride),
        #            get_same_padding(w, self.kernel, self.dilation, self.stride))
...
#         k - kernel
#         """
#         b, d, h, w = fmap.shape[0], fmap.shape[-3], fmap.shape[-2], fmap.shape[-1]
...
#         # l2 distance or dot product
#         sim = torch.einsum('bij,bjk->bik', q, k)
#         sim = sim * self.scale
#         # attention
#         w_ = sim.softmax(dim = -1)
#         w_ = rearrange(w_, 'b i j -> b j i')
#         h_ = torch.einsum('bij,bjk->bik', v, w_)
#         out = rearrange(h_, 'b c (h w d) -> b c h w d', h = x, w = y, d = z)
...
#     def __init__(self, dim):
#         super().__init__()
#         self.weight = nn.Parameter(torch.zeros(dim, 1, 1, 1))
...
#         noise = None
#     ):
#         b, _, h, w, d, device = *x.shape, x.device

#         if not exists(noise):
#             noise = torch.randn(b, 1, h, w, d, device = device)
...
class Generator(BaseGenerator):
    ...
    def __init__(
        ...
        *,
        ...
        # partial(nn.Conv3d, kernel_size = 3, padding=1)
        # adaptive_conv = conv3d_3x3
        adaptive_conv = partial(AdaptiveConv3DMod, kernel = 3, num_conv_kernels = num_conv_kernels)

        # initial 4x4x4 block and conv

        # self.init_block = nn.Parameter(torch.randn(dim_latent, 4, 4, 4))
        self.ls = latent_size
        ...
        for ind, ((dim_in, dim_out), resolution) in enumerate(zip(dim_pairs, resolutions)):
            ...
            # to_rgb = AdaptiveConv3DMod(dim_in, channels, 1, num_conv_kernels = 1, demod = False)
            # to_rgb = AdaptiveConv3DMod(dim_out, channels, 1, num_conv_kernels = 1, demod = False)
        ...
        # self.out_conv = nn.Conv3d(16, channels, 1)
    ...
    def forward(
        ...
        x = repeat(self.init_block, 'c h w d -> b c h w d', b = batch_size)
        ...
        rgbs = []
        # for squeeze_excite, (resnet_conv1, noise1, act1), to_rgb_conv, self_attn, cross_attn, upsample, upsample_rgb in self.layers:
...
class Discriminator(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        self.to_logits = nn.Sequential(
            conv3d_3x3(dim_last, dim_last),
            leaky_relu(),
            gap3d(),
            flatten,
            nn.Linear(dim_last, 1),
            # Rearrange('b c h w d -> b (c h w d)'),
            # nn.Linear(dim_last * (8 ** 2), 1),
            Rearrange('b 1 -> b')
        )
    ...
    def forward(
        ...
        x = images
        # texts = torch.tensor(texts).unsqueeze(-1).to(self.device)
    ...
    def save_sample(
        ...
        for model, filename in sample_models_and_output_file_name:
            ...
            for i, image_numpy in enumerate(all_images_numpy):
                ...
                if str(self.results_folder).split('/')[-1] == 'brats' or str(self.results_folder).split('/')[-1] == 'brats-pixelshuffle':
                    ...
                    sitk.WriteImage(sitk.GetImageFromArray(image_numpy[2]), os.path.join(self.results_folder, f'{filename}_prob2_{i}.nii.gz'))
                elif str(self.results_folder).split('/')[-1] == 'lung' or str(self.results_folder).split('/')[-1] == 'lung-pixelshuffle':
                    save_numpy[image_numpy[0] > 0.5] = 1
