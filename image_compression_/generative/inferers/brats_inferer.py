...
class DiffusionInferer(Inferer):
    ...
    def sample(
        ...
        moving = input_noise[:,:half]
        if time_start is None:
            time_start = 0
        if verbose and has_tqdm:
            progress_bar = tqdm(scheduler.timesteps[time_start:])
        else:
            progress_bar = iter(scheduler.timesteps[time_start:])
        intermediates = []
        for t in progress_bar:
            ...
            image, _ = scheduler.step(model_output, t, image[:,half:])
            image = torch.cat([moving, image], dim=1)
